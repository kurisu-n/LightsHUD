    ...
    function enableRelevantButtons() {
      ...
      if (!noCheck)
        noCheck =
          ...
          (!data.isGM && !game.settings.get("torchlight", "checkAvailability"));
      ...
      if (
        ...
        (hasItemInInventory("Oil (flask)") &&
          (hasItemInInventory(game.settings.get("torchlight", "nameHoodedLantern"))) ||
            hasItemInInventory(game.settings.get("torchlight", "nameBullseyeLantern")))
      )
      ...
      if (tbutton === tbuttonLantern) {
        ...
        if (statusLantern) {
          ...
          if (consumeItem("Oil (flask)")) {
            ...
            //});
			ui.notifications.error("No Oil (flask) in Inventory !")
            disableTorchlightButton(tbuttonLantern);
          }
        }
      ...
      if (tbutton === tbuttonTorch) {
        ...
        if (statusTorch) {
          ...
          if (consumeItem("Torch")) {
            ...
            //});
			ui.notifications.error("No Torch in Inventory !")
            disableTorchlightButton(tbuttonTorch);
          }
        }
      }
...
*/
    ...
    function hasItemInInventory(itemToCheck) {
      ...
      actor.data.items.forEach((item) => {
        if (item.name.toLowerCase() === itemToCheck.toLowerCase()) {
          if (item.data.data.quantity > 0) hasItem = true;
        }
      });
      return hasItem;
    }
    ...
    function consumeItem(itemToCheck) {
	  ...
	  TorchLight.log("Consume 1: "+consume);
      if (consume)
	  { let consumeBypass =
          (data.isGM && !game.settings.get("torchlight", "dmAsPlayer")) ||
		  (!data.isGM && !game.settings.get("torchlight", "consumeItem"));
		  if (consumeBypass)
			  return true;
	  }
        ...
        actor.data.items.forEach((item) => {
          if (item.name.toLowerCase() === itemToCheck.toLowerCase()) {
            if (item.data.data.quantity > 0) {
			  ...
			  objUpdate["data.quantity"] = item.data.data.quantity - 1;
			  item.data.update(objUpdate);
	  ...
	  TorchLight.log("Consume 3: "+consume);
...
Hooks.once("init", () => {
  ...
  // Lantern Parameters
    if (game.system.id === "dnd5e") {
		game.settings.register("torchlight", "nameHoodedLantern", {
      name: game.i18n.localize("torchlight.nameHoodedLantern.name"),
      hint: game.i18n.localize("torchlight.nameHoodedLantern.hint"),
      scope: "world",
      config: true,
      default: "Hooded Lantern",
      type: String,
    });
	    game.settings.register("torchlight", "nameBullseyeLantern", {
      name: game.i18n.localize("torchlight.nameBullseyeLantern.name"),
      hint: game.i18n.localize("torchlight.nameBullseyeLantern.hint"),
      scope: "world",
      config: true,
      default: "Bullseye Lantern",
      type: String,
    });
  }
